...
public enum HomeImg {
    ...
    MOUNTAIN("피톤치드 향 가득 느낄 수 있는 건강식 어때요"),
    SUMMER("무더운 여름엔 이열치열로 땀좀 빼 볼까요", "따가운 햇살엔 이열치열! 뜨거운 음식들", "햇살이 뜨거울때 차가운 음식들로 이겨내 볼까요"),
    WINTER("추운 겨울엔 뜨끈한 국물 어때요", "추운 겨울의 따끈따근한 제철 음식이에요"),
    ;

    private final List<String> imgUrlList = new ArrayList<>();

    HomeImg(String... strings) {
    ...
    public String getRandomImgUrl() {
        ...
        return imgUrlList.get(randomValue);
    }
}